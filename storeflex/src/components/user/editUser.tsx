...
import { UserType, StatusType } from '../atoms/adduser/UserHelper';
...
const EditUser = () => {
  ...
  const onStatusTypeUpdate = (statusType: string) => {
    setUserPostInfo({ ...userPostInfo, status: statusType });
    console.log(' << onStatusTypeUpdate >> ', statusType);
  }
      ...
      <Grid container spacing={2} columns={{ xs: 4, sm: 12, md: 12 }}>
        <Grid item xs={2.5}>
          ...
          <div className='p-top-md'>
            {<StatusType defaultStatus='' onUpdate={onStatusTypeUpdate} />}
          </div>
        </Grid>
        <Grid item xs={5} />
        <Grid item xs={5}>
